...
ui.play.onclick=()=>{running=true;ui.start.style.display="none";if(!isTouchDevice)renderer.domElement.requestPointerLock?.()};renderer.domElement.onclick=()=>{if(!isTouchDevice&&running&&!locked&&!inventoryOpen&&!backpackOpen)renderer.domElement.requestPointerLock?.()};document.addEventListener("pointerlockchange",()=>{locked=document.pointerLockElement===renderer.domElement;if(!locked)cancelMining();setHotbarReorderHint()});
